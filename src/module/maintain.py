...
reload_conf = on_command("reload_conf", aliases={"配置重载"}, permission=SUPERUSER, rule=to_me(),priority=0,block=True)
@reload_conf.handle()
async def reply_reload_conf():
    Constants.reload_conf()
    Constants.log.info("[obot-core] 已重载配置文件")
    await MessageFactory("已重载配置文件").finish()
...
async def reply_chat_scene_id(event:Event):
    await MessageFactory(f'当前对话场景ID\n\n{event.get_session_id()}').finish()

my_id = on_command("my_id", aliases={"我的ID"}, rule=to_me(),priority=5,block=True)
@my_id.handle()
async def reply_my_id(event:Event):
    await MessageFactory(f'你的ID（不同对话场景下你的ID是不同的）\n\n{event.get_user_id()}').finish()
