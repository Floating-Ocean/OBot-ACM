...
today_contests_1 = on_command(("contest","today"),
                              aliases={("contests","today"),("比赛","today"),("比赛","今日"),("比赛","今天")},priority=50,block=True,rule=to_me())
today_contests_2 = (on_command("今日比赛",aliases={"今日的比赛", "今天比赛", "今天的比赛"}, priority=50, block=True, rule=to_me()))
recent_contests_1 = on_command(("contest","recent"),
                               aliases={("contests","recent"),("比赛","近期"),("比赛","最近")},priority=50,block=True,rule=to_me())
...
async def reply_contests(event:Event,platform:str,is_today:bool):
    ...
    await reply([f"以下是当前查询到的{tip_time_range}比赛列表",png2jpg(f"{cached_prefix}.png")], event, modal_words=False, finish=True)


generate_qr = on_command("qrcode", aliases={"qr", "二维码", "码"}, priority=100, block=True)
@generate_qr.handle()
...
sleep = on_command("sleep", aliases={"晚安", "睡觉", "睡觉去了"}, priority=200, block=True)
@sleep.handle()
...
help_command = on_command("help", aliases={"helps", "instruction", "instructions", "帮助"}, priority=100, block=True)
@help_command.handle()
...
about_command = on_command("about", aliases={"版本", "关于"}, priority=100, block=True)
